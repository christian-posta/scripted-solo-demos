...
tmux send-keys -t 1 "kubectl port-forward deployments/productpage-v1 9080 --cluster $CLUSTER_1" C-m
...
run "kubectl get authorizationpolicies -A  --cluster $CLUSTER_1"
run "kubectl get authorizationpolicies global-access-control -o yaml -n istio-system --cluster $CLUSTER_1"
