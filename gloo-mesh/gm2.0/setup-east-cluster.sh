...
helm install gloo-mesh-agent gloo-mesh-agent/gloo-mesh-agent --kube-context=$CLUSTER_2 --version=$GLOO_MESH_VERSION --namespace gloo-mesh-gateway --set glooMeshAgent.enabled=false --set rate-limiter.enabled=true --set ext-auth-service.enabled=true
