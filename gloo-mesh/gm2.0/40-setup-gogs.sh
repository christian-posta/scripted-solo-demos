...
kubectl --context $MGMT create ns gogs
kubectl --context $MGMT create cm custom-gogs -n gogs --from-file ./resources/gitops/gogs/app.ini
kubectl --context $MGMT apply -f ./resources/gitops/gogs/gogs.yaml
kubectl --context $MGMT rollout status -n gogs deploy/gogs

kubectl --context $MGMT apply -f ./resources/gloo/gogs-vs.yaml
...
kubectl --context $MGMT -n gogs exec -it deploy/gogs -- /bin/sh -c 'gosu git ./gogs admin create-user --name ceposta --password admin123 --email christian@solo.io --admin'


kubectl --context $MGMT port-forward -n gogs deploy/gogs 3000 &> /dev/null &
PF_PID="$!"
...
echo "FATAL::Check this.... we are failing here because we don't want to set up the repo yet.."
