...
if [ "$USING_KIND" == "true" ] ; then
    helm install gloo-mesh-enterprise ./charts/gloo-mesh-enterprise \
    --kube-context $MGMT -n gloo-mesh  \
    --set licenseKey=${GLOO_MESH_LICENSE} \
    --set glooMeshMgmtServer.ports.healthcheck=8091 \
    --set glooMeshMgmtServer.image.registry=docker.io \
    --set glooMeshMgmtServer.image.repository=ilackarms/gm-mgmt-server \
    --set glooMeshMgmtServer.image.tag=latest \
